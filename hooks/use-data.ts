...
export async function syncCreditAccountCycle(creditAccountId: string) {
  ...
  await supabase
    ...
    .update({
      ...
      available_credit_dop: Math.max(0, Number(account.credit_limit_dop || 0) - Number(account.current_debt_dop || 0)),
      available_credit_usd: Math.max(0, Number(account.credit_limit_usd || 0) - Number(account.current_debt_usd || 0)),
