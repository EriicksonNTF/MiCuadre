...
export const FINANCIAL_SUBSCRIPTION_PROVIDERS = [
...
export function getFinancialSubscriptionProvider(providerKey?: string | null) {
  return FINANCIAL_SUBSCRIPTION_PROVIDERS.find((provider) => provider.key === providerKey) || FINANCIAL_SUBSCRIPTION_PROVIDERS[FINANCIAL_SUBSCRIPTION_PROVIDERS.length - 1]
}

export function getNextFinancialBillingDateFrom(baseDate: Date, billingDay: number) {
